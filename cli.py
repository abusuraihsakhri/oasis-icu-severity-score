"""
Command Line Interface for Oasis Icu Severity Score.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="oasis-icu-severity-score", description="Oasis Icu Severity Score")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  OASIS ICU SEVERITY SCORE")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Oasis Icu Severity Score Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "batch":
        from oasis_score import _validate_csv_path
        input_path = _validate_csv_path(args.input, must_exist=True)
        output_path = _validate_csv_path(args.output, must_exist=False)
        ...
        for r in rows:
            try:
                ...
                print(f"Error: Invalid row data for task_id={r.get('task_id', '?')}: {e}", file=sys.stderr)
                return 1
            dossier = supervisor.process_task(payload)
            row_dict = dict(r)
    ...
    if args.command == "serve":
        ...
        print(f"Starting Oasis Icu Severity Score API server on http://{args.host}:{args.port}")
